...
export default function EditCustomerPage() {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && currentCompanyId) {
      apiClient.get(`/companies/${currentCompanyId}/customers/${params.id}`).then(({ data }) => {
        setName(data.name);
        setEmail(data.email ?? '');
        setPhone(data.phone ?? '');
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setIsSaving(true);
    try {
      await apiClient.patch(`/companies/${currentCompanyId}/customers/${params.id}`, { name, email: email || undefined, phone: phone || undefined, paymentTermDays });
  ...
  return (
    <main style={{ maxWidth: 600, margin: '4rem auto' }}>
      ...
      <p>
        <a href={`/customers/${params.id}`}>Annuler</a>
      </p>
    </main>
  );
}
