...
export default function NewCustomerPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setIsSaving(true);
    try {
      const { data } = await apiClient.post(`/companies/${currentCompanyId}/customers`, {
        code,
        name,
        ...
        <div>
          ...
          {accounts.length === 0 && (
            <p style={{ fontSize: '0.85em', color: '#a15c00' }}>
              Aucun compte de tiers trouvé — créez-en un dans le plan comptable avant de créer un client.
            </p>
          )}
        </div>
        <div>
