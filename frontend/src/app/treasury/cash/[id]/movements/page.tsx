...
export default function CashMovementsPage() {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && currentCompanyId) {
      apiClient
        .get(`/companies/${currentCompanyId}/cash-accounts/${params.id}/movements`)
        .then(({ data }) => setMovements(data.transactions))
        .catch(() => setError('Impossible de charger les mouvements (permission CASH.MOVEMENT.READ requise).'));
    }
  ...
  return (
    <main style={{ maxWidth: 800, margin: '4rem auto' }}>
          ...
          <tbody>
            {movements.map((m) => (
              <tr key={m.id}>
                ...
                <td>{m.type === 'RECEIPT' ? 'Entrée' : 'Sortie'}</td>
                <td>{m.label}</td>
                <td>{m.reference ?? '—'}</td>
      ...
      <p style={{ marginTop: '1rem' }}>
        <a href={`/treasury/cash/${params.id}`}>Retour à la caisse</a>
      </p>
    </main>
  );
}
