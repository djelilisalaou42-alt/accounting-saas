...
export default function NewBankAccountPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setIsSaving(true);
    try {
      const { data } = await apiClient.post(`/companies/${currentCompanyId}/bank-accounts`, {
        code,
        name,
        bankName,
        iban: iban || undefined,
      ...
      router.push(`/treasury/banks/${data.id}`);
    } catch (err) {
    ...
    <main style={{ maxWidth: 600, margin: '4rem auto' }}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div>
          ...
          <br />
          <input id="swiftBic" value={swiftBic} onChange={(e) => setSwiftBic(e.target.value)} />
        </div>
        <div>
        ...
        <button type="submit" disabled={isSaving} style={{ marginTop: '1rem' }}>
          {isSaving ? 'Création…' : 'Créer le compte bancaire'}
        </button>
      </form>

      <p>
        <a href="/treasury/banks">Annuler</a>
      </p>
    </main>
  );
}
